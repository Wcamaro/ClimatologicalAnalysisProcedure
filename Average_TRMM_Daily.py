...
from process_trmm_3b42_daily import init,  WriteGTiff_media, cumulatedict, calendardays, media
import datetime
...
for cumulated in cumulateds: # Number of months to cumulate; 
    ...
    for dat in months:
        ...
        if dict_matrix == {}:
            ...
            (dict_media[str(dat)],dict_nmat[str(dat)]) = media(dict_matrix)
    folder_output = r'D:\Python_Projects\Results\TRMM\Average_TRMM_Daily\%s' % (cumulated)
    if not os.path.exists(folder_output):
        os.makedirs(folder_output)
    WriteGTiff_media(dict_media,folder_output,'Average_TRMM_Daily_%smonthly' % cumulated,bbox['left'],bbox['right'],bbox['bottom'],bbox['top'],end_date.day)
